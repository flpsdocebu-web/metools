...
if($route==='draft'&&$method==='POST'){ $u=current_user($d);if($u['role']==='admin')out(['error'=>'Administrator cannot save school drafts.'],400);$b=body();$data=$b['data']??[];$data['score']=score($data['checklist']??[]);$d['drafts'][$u['id']]=$data;save_data($d);out(['ok'=>true,'score'=>$data['score']]); }
