...
export default async (req,context)=>{
  ...
  try{
    ...
    if(path==="/submit"&&req.method==="POST"){
      ...
      await blobStore().setJSON(`report-${id}`,report);const idx=await getSubIndex();const data=report.data||{};idx.unshift({id,userId:u.id,district:u.district,schoolName:u.schoolName,schoolId:u.schoolId,submittedAt:report.submittedAt,status:report.status,score:data.score||null,monitoringDate:data.monitoringDate||"",hazards:(data.emergencies||[]).map(x=>x.hazardType).filter(Boolean),continuityLevels:(data.continuityActivations||[]).map(x=>x.level).filter(Boolean),technicalAssistanceStatuses:(data.technicalAssistanceRecords||[]).map(x=>x.taStatus).filter(Boolean)});await setSubIndex(idx.slice(0,5000));return json({ok:true,id});
    }
